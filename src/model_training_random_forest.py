...
config = load_config()
setup_logging(config['data']['log_path'] + 'model_training_random_forest.log')
...
def train_random_forest_model():
    try:
        ...
        # Define RandomForest parameter grid from config.yaml
        param_grid = {
        ...
        grid_search = RandomizedSearchCV(estimator=rf, param_distributions=param_grid, n_iter=50, cv=5, n_jobs=-1, scoring='f1', random_state=42)
        ...
        plot_confusion_matrix(y_test, y_pred, model_name='RandomForest', config=config)

        # Save the trained model
        save_model(best_rf, 'random_forest', config)

    except Exception as e:
        logging.error(f"Error during Random Forest model training: {str(e)}")
        raise
